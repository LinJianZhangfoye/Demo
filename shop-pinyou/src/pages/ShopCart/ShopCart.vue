<template>
  <div class="cart">
    ...
    <div class="cart-main">
      ...
      <div class="cart-body">
        <ul
          class="cart-list" v-for="(cart,index) in cartInfoList" :key="index">
          ...
          <li class="cart-list-con5">
            <a
              href="javascript:void(0)" class="mins" @click="handle('mins',-1,cart)">-</a>
            <input
              autocomplete="off" type="text" minnum="1" class="itxt" :value="cart.skuNum" @change="handle('change',$event.target.value*1,cart)" />
    ...
    <div class="cart-tool">
      <div class="select-all">
        <input class="chooseAll" type="checkbox" @change="updateAllCartChecked" :checked="isAllCheck && cartInfoList.length>0"/>
        <span>全选</span>
      </div>
      <div class="option">
...
export default {
  ...
  computed:{
      ...mapGetters(['cartNewList']),
      cartInfoList(){
          return this.cartNewList.cartInfoList || []
      },
      //判断全选按钮是否勾选
      isAllCheck() {
  ...
  methods: {
        ...
        switch(type){
            ...
            case 'change':
                if(isNaN(disNum)||disNum<1){
                    ...
                    //属于正常情况，如果带来的是小数需要取整，带给服务器的是变化的量
      ...
      //删除某个产品的操作
      deleteCartById:throttle( async function(index){
        try{
            await this.$store.dispatch('deleteCartListBySkuId',index)
            this.getData()
        } catch{
            alert(error.message)
        }
